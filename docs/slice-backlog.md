...
| S16.02 | 16 | Relocate secrets out of the repo working tree | Claude Code | Codex CLI | review_ready | Closes audit finding P2-① (the live secret files lived inside the public-repo checkout, protected only by `.gitignore`). **Acceptance criteria:** (1) the live `*.secret` files are moved to the canonical `~/talisman/secrets/` (dir 0700, files 0600), verified by name+size; (2) the in-repo `secrets/` directory no longer exists; (3) `.gitignore` keeps `secrets/` / `*.secret` as belt-and-suspenders; (4) no committed runtime consumer breaks (the canonical config already uses `~/talisman/secrets/`; the dated audit-package snapshot is left intact); (5) deterministic checks stay green (docs/ops only). |
