...
| S16.06 | 16 | Containerized worker containment (the real egress boundary) | Claude Code | Codex CLI | review_ready | **Part 2 of P0-B** — the containment that makes AT-14 a real boundary; per **ADR-0007** (Pat: index on full autonomy → containerized, proxy-only-networked workers). **Acceptance criteria:** (1) `workers/_container.py` `ContainerRunner` is a drop-in `CommandRunner` running a worker in a rootless-podman container on an `--internal` network — proxy-only env, workspace-only mount, no provider keys; (2) unit tests cover the podman invocation (always run); (3) an integration test spins up a real container on an `--internal` network and proves it CANNOT egress (self-skips where podman/internal-nets are unavailable, so the gate never depends on a runtime); (4) AT-14 stays COMPONENT_VERIFIED — the mechanism is proven but the orchestrator doesn't run workers through it yet (no inflation; flips to PASS in the wiring follow-up); (5) all five checks green. |
