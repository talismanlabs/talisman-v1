...
| S16.08 | 16 | Gateway retry with full jitter (AT-12) | Claude Code | Codex CLI | review_ready | Hardens the v1-waived **AT-12 → PASS** (pulled forward for autonomy: a transient API blip must not kill an unattended run). **Acceptance criteria:** (1) `gateway_client.retrying_transport` wraps any transport with full-jitter exponential backoff, retrying transport errors + 429/5xx and honoring a numeric Retry-After; non-retryable 4xx surface immediately; wired into `GatewayClient.over_http`; (2) CI tests cover retry-then-succeed, no-retry-on-4xx, Retry-After honored, exhaustion re-raises, and jitter bounded by the cap (injected sleep/rand for determinism); (3) `app/release` AT-12 WAIVED → PASS; locking tests + checklist updated (now 8 PASS / 9 component / 3 waived); (4) all five checks green. |
