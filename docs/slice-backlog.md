...
| S16.07 | 16 | Durable checkpointer — resume survives restart (AT-04) | Claude Code | Codex CLI | review_ready | Hardens the v1-waived **AT-04 → PASS** (pulled forward for autonomy: unattended runs must survive crashes). **Acceptance criteria:** (1) add `langgraph-checkpoint-sqlite` + `composition.build_sqlite_checkpointer(db_path)` wiring a durable SqliteSaver; (2) a CI test pauses a gate under one checkpointer and resumes under a BRAND-NEW checkpointer on the same on-disk DB (recovered from disk), with a MemorySaver contrast proving it is real; (3) `app/release` AT-04 WAIVED → PASS with that test as evidence; locking tests + checklist updated (now 7 PASS / 9 component / 4 waived); (4) the in-memory default is unchanged for dry-run/tests; (5) all five checks green. |
