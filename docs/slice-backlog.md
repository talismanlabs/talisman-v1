...
| S16.05 | 16 | Fix the Codex worker invocation (prompt on stdin) | Claude Code | Codex CLI | review_ready | Closes the Codex-invocation gap (audit P2-⑧ + AT-08 readiness). **Acceptance criteria:** (1) the shared runner accepts optional `stdin_text` and feeds it to the child; (2) `codex_cli` runs `codex exec --skip-git-repo-check -` with the prompt on stdin (out of ps/proc; the invocation Codex CLI requires), `claude_code` unchanged (prompt still argv); (3) a real-subprocess test proves stdin reaches the child, and the worker contract tests move to the 4-arg runner; (4) `app/release` AT-08 evidence updated but stays COMPONENT_VERIFIED (flips to PASS only on a live Codex run, not exercisable in CI) — no grade inflation; (5) all five checks green. |
