...
def test_codex_cli_worker_satisfies_worker_contract(worker_request, assert_worker_contract) -> None:
    ...
    worker = CodexCliWorker(runner=lambda args, cwd, timeout: CommandResult(0, "ran ok"))
...
def test_codex_cli_worker_invokes_cli_in_workspace(worker_request) -> None:
    """The adapter builds a `codex exec <prompt>` command and runs it in the workspace."""
    captured: dict[str, object] = {}

    def fake_runner(args: list[str], cwd: Path, timeout: int) -> CommandResult:
        captured["args"] = args
        captured["cwd"] = cwd
        return CommandResult(exit_code=0, stdout="done")

    CodexCliWorker(runner=fake_runner).run(worker_request)
    assert captured["args"] == ["codex", "exec", "Implement the slice."]
...
def test_codex_cli_worker_propagates_nonzero_exit(worker_request) -> None:
    ...
    worker = CodexCliWorker(runner=lambda args, cwd, timeout: CommandResult(2, "boom"))
